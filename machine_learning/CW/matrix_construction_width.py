...
#for x in range(y):
    ...
    SN = np.array(clusterDF["S/N"], dtype = np.float64).round(2)
    WIDTH = np.array(clusterDF["Width"], dtype = np.float64).round(1)
    ...
    zero[indices,v.astype(int)] = tupleArr[dmArange]
    #
    #print(zero)
    #break
