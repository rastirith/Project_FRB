...
for i in range(0,5): 
    ...
    for q in range(1,len(labels)):
        ...
        # All candidates that haven't been excluded thus far go in here
            ...
            ax.scatter(dmData, snData, marker = 'o', s = 5, label = "Detected events")
            ax.set_xlabel("DM (pc " + r'$cm^-3$' + ")")
            ax.set_ylabel("SN")
            ax.set_title("Real burst")
            lgnd = plt.legend()
            ...
            logX = np.log([0.192, 2.176, 4.352, 8.704, 17.408])
            logY = np.log([169.632, 54, 36.4, 22, 13.75])
            
            xRange = np.arange(0,35, 0.01)
            m,b = np.polyfit(logX, logY, 1)
            fit = np.log(xRange)*m + b
            ...
            fig1 = plt.figure()
            ax1 = fig1.add_subplot(111)
            ax1.scatter(logX, logY, marker = 'o', s = 5, label = "Detected events", alpha = 0.4)
            ax.plot(xRange, Y, label = "Detected events", alpha = 0.5)
            ax1.set_xlabel("DM (pc " + r'$cm^-3$' + ")")
            ax1.set_ylabel("SN")
            ax1.set_title("SN vs DM of Crab pulse")
            ax.set_ylim(0,175)"""
            ...
            for i in reversed(range(len(np.unique(widthData)))):
                ...
                colors = np.full((len(a)), np.unique(widthData)[i])
                print(colors)
                print((np.unique(widthData)[i]))
                ...
                b = ax.scatter(dmData[a], snData[a], vmin = -1, vmax = 20, label = str(np.unique(widthData)[i]), cmap = "brg", c = colors, s = 6)#, s = 25*((np.unique(widthData)[i])))
            ...
            """
            cbar = plt.colorbar(b)
            cbar.set_label("Width (ms)")
            cbar.set_ticks([2.5, 5, 7.5, 10, 12.5, 15.0, 17.5, 20.0])
            cbar.set_ticklabels(["2.5", "5", "7.5", "10", "12.5", "15.0", "17.5", ">20.0"])"""
