...
while counter < numBursts:
    ...
    if max(data[:,0]) < 170:
        ...
        upperTime = np.amax(data[:,1])
        upperDM = np.amax(data[:,0])
        ...
        dmMid = np.random.uniform(dmRangeBot, upperDM)       # Average DM of the burst
        ...
        if intention == "s":
            ...
            ax.scatter(finalArr[:,0],finalArr[:,2], s = 5, c = finalArr[:,4], cmap = "brg", vmin = -1, alpha = 0.6)
            ax.set_xlabel("Time")
            ax.set_ylabel("DM")
...
while (counter < numBursts) and ((intention == "t") or (intention == "s")):
    progressBar(counter, numBursts)
    ...
    if max(data[:,0]) < 100:
        ...
        if funcVar > 0:
            ...
            tempDM = rescale(dataArr[0], dmMid, dmMid + np.amax(dataArr[0]) - np.amin(dataArr[0]))
            snArr = dataArr[1]
