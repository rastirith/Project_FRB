...
check_step = 0.1
check_range = 1
checksteps_1d = int(check_range/check_step)
check_iterations = int((checksteps_1d + 1)**2)
...
score_struct = []
check_struct = []
TP_iter = np.zeros((int(1/step), check_iterations))
FP_iter = np.zeros((int(1/step), check_iterations))
TN_iter = np.zeros((int(1/step), check_iterations))
FN_iter = np.zeros((int(1/step), check_iterations))

for i in range(checksteps_1d + 1):
    a = i*check_step
    print(a)
    for k in range(checksteps_1d + 1):
        b = k*check_step
        check_struct.append([a,b])

check_struct = np.array(check_struct)
    ...
    for q in range(1,len(np.unique(clusters))):
        ...
        if (max_ind > 4) or (max_ind < 2):
           ...
           #developing peak shape conditions
            ...
            for h in range(len(check_struct)):
                ...
                weight_3 = 1
                weight_4 = -1
                check_1 = check_struct[h][0]
                check_2 = check_struct[h][1]
                #check_3 = 0.25
                score = [0,1.3,2.5,2.5]
...
xTP = np.zeros((int(1/step),check_iterations))
xFN = np.zeros((int(1/step),check_iterations))
xTN = np.zeros((int(1/step),check_iterations))
xFP = np.zeros((int(1/step),check_iterations))

for i in range(len(TP_iter)):
    for h in range(check_iterations):
...
for h in range(check_iterations):
    ...
    if (dist_avg  > max_avg):
        ...
        #print(h)
        #print(dist_avg)
        #print(check_struct[h])
"""   
dist = 0   
for i in range (len(TP_iter)):       
    if ((xTP[i][check_setting] - xFP[i][check_setting]) > dist):
        dist = (xTP[i][h] - xFP[i][h])
        conf_setting = i"""

print("Low limit: " + str(100*conf_setting*step) + "%")
print("Score setting: " + str(check_struct[check_setting]))
